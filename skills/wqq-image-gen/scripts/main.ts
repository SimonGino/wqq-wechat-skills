...
Options:
  ...
  --n <count>               Number of images (Imagen models only, default: 1)
  --json                    JSON output
  -h, --help                Show help
...
async function main(): Promise<void> {
  ...
  // Generate image with retry and exponential backoff
  const imageData = await retryWithBackoff(
    () => providerModule.generateImage(prompt, model, args),
    {
      maxAttempts: 3,
      delayMs: 1000,
      backoffFactor: 2,
      onRetry: (error, attempt) => {
        console.error(
          `Generation failed (attempt ${attempt}/3): ${formatError(error)}`,
        );
...
main().catch((e) => {
  ...
  if (e instanceof Error) {
    if (e.message.includes("API key")) {
      console.error("\nTip: Set your API key in ~/.wqq-skills/.env");
      console.error("Example: OPENAI_API_KEY=sk-xxx or GOOGLE_API_KEY=xxx");
    } else if (
      e.message.includes("ENOENT") ||
      e.message.includes("not found")
    ) {
      console.error(
        "\nTip: Check that all file paths exist and are accessible",
      );
    }
  }
