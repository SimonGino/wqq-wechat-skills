...
function getGoogleImageSize(args: CliArgs): "1K" | "2K" | "4K" {
  if (args.imageSize) return args.imageSize;
...
function extractPredictedImageData(response: { predictions?: Array<any>; generatedImages?: Array<any> }): string | null {
...
async function generateWithImagen(prompt: string, model: string, args: CliArgs): Promise<Uint8Array> {
  ...
  const response = await postGoogleJson<{ predictions?: Array<any>; generatedImages?: Array<any> }>(
...
export async function generateImage(prompt: string, model: string, args: CliArgs): Promise<Uint8Array> {
  if (isGoogleImagen(model)) {
    ...
    return await generateWithImagen(prompt, model, args);
  }

  if (!isGoogleMultimodal(model)) {
    console.error("Warning: Unknown Google model type, defaulting to Gemini multimodal generateContent.");
  }

  return await generateWithGemini(prompt, model, args);
}
